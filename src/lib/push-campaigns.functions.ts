...
const campaignInput = z.object({
  ...
  audienceType: z.enum(["all", "new", "active", "recurring", "inactive", "birthday", "abandoned_cart", "post_purchase"]),
  ...
  recurrenceTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).default("10:00"),
...
async function assertPermission(
  ...
  if (error || !data) throw new Error("Você não tem permissão para gerenciar notificações desta loja.");
}
...
function mapCampaign(row: Record<string, unknown>): PushCampaignSummary {
  return {
    ...
    removedCount: Number(row["removed_count"] ?? 0), createdAt: String(row["created_at"]),
  };
}
...
export const getPushCampaignOverview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: campaigns, error }, { count: subscribers }] = await Promise.all([
      context.supabase.from("push_campaigns").select("*").eq("store_id", data.storeId).order("created_at", { ascending: false }).limit(100),
      context.supabase.from("push_subscription_stores").select("id", { count: "exact", head: true }).eq("store_id", data.storeId).eq("is_active", true).not("consented_at", "is", null),
    ]);
...
export const savePushCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertPermission(context, data.storeId);
    if (data.scheduleType === "scheduled" && !data.scheduledAt) throw new Error("Escolha a data e a hora do envio.");
    if (data.scheduleType === "recurring" && data.recurrenceDays.length === 0) throw new Error("Escolha ao menos um dia da semana.");
    ...
    const query = data.id
      ? context.supabase.from("push_campaigns").update(values).eq("id", data.id).eq("store_id", data.storeId)
      : context.supabase.from("push_campaigns").insert(values);
    const { data: row, error } = await query.select("id").single();
    if (error) throw new Error(error.message);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.from("audit_logs").insert({ store_id: data.storeId, user_id: context.userId, action: data.id ? "push_campaign.updated" : "push_campaign.created", entity: "push_campaigns", entity_id: row.id, metadata: { status: values.status, schedule_type: data.scheduleType } });
    return { id: row.id, status: values.status };
  });

const actionInput = z.object({ storeId: z.string().uuid(), campaignId: z.string().uuid(), action: z.enum(["pause", "resume", "cancel", "duplicate"]) });
export const updatePushCampaignStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => actionInput.parse(input))
  .handler(async ({ data, context }) => {
    await assertPermission(context, data.storeId);
    const { data: campaign, error } = await context.supabase.from("push_campaigns").select("*").eq("id", data.campaignId).eq("store_id", data.storeId).single();
    ...
    if (data.action === "duplicate") {
      ...
      const { error: insertError } = await context.supabase.from("push_campaigns").insert({ ...copy, name: `${campaign.name} — cópia`.slice(0, 80), created_by: context.userId, status: "draft", next_run_at: null, last_run_at: null, sent_count: 0, failed_count: 0, removed_count: 0 });
      if (insertError) throw new Error(insertError.message);
    } else {
      const status = data.action === "pause" ? "paused" : data.action === "cancel" ? "cancelled" : "scheduled";
      const { error: updateError } = await context.supabase.from("push_campaigns").update({ status, next_run_at: data.action === "resume" ? new Date().toISOString() : campaign.next_run_at }).eq("id", data.campaignId);
      if (updateError) throw new Error(updateError.message);
    }
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.from("audit_logs").insert({ store_id: data.storeId, user_id: context.userId, action: `push_campaign.${data.action}`, entity: "push_campaigns", entity_id: data.campaignId, metadata: {} });
    return { ok: true };
  });

export const sendCustomerPushTest = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ storeId: z.string().uuid(), title: z.string().trim().min(2).max(80), body: z.string().trim().min(2).max(240) }).parse(input))
  .handler(async ({ data, context }) => {
    await assertPermission(context, data.storeId);
    const { sendPush } = await import("@/lib/push.server");
    return sendPush(context.supabase, { userIds: [context.userId] }, { title: data.title, body: data.body, url: "/painel/notificacoes", tag: "push-campaign-test" });
  });
