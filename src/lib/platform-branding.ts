...
export async function fetchPlatformBranding(): Promise<PlatformBranding> {
  const { data, error } = await supabase
    ...
    .select("sales_logo_light_url, sales_logo_dark_url, merchant_logo_light_url, merchant_logo_dark_url, favicon_url, app_cover_url, pwa_icon_url, pwa_maskable_icon_url, updated_at")
    .eq("key", "default")
    .maybeSingle();
