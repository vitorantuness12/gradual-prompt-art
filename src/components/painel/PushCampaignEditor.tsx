...
export interface PushCampaignEditorProps {
  busy: boolean;
  onSave: (draft: PushCampaignDraft) => void;
  onTest: (content: { title: string; body: string }) => void;
}
const DAYS = [
  { value: 0, label: "Dom" },
  { value: 1, label: "Seg" },
...
export function PushCampaignEditor({ busy, onSave, onTest }: PushCampaignEditorProps) {
  ...
  function readForm(event: FormEvent<HTMLFormElement>) {
    ...
    onSave({
      ...
      body,
      audienceType,
      inactiveDays: Number(form.get("inactive_days") ?? 60),
      scheduleType,
      scheduledAt: String(form.get("scheduled_at") ?? "")
        ...
        <form className="grid gap-5" onSubmit={readForm}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={audienceType}
                onValueChange={(value) => setAudienceType(value as PushAudienceType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {PUSH_AUDIENCES.map((item) => (
          ...
          {audienceType === "inactive" ? (
            <div className="space-y-2">
              <Label htmlFor="inactive-days">Sem comprar há quantos dias?</Label>
              <Input
                id="inactive-days"
                name="inactive_days"
                type="number"
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={80}
              placeholder="Uma novidade para você"
              required
            />
          </div>
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-xs text-muted-foreground">{body.length}/240</span>
            </div>
            <Textarea
              id="push-body"
              value={body}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select
                value={scheduleType}
                onValueChange={(value) => setScheduleType(value as PushScheduleType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {PUSH_SCHEDULES.map((item) => (
                    <SelectItem key={item.value} value={item.value}>
                      {item.label}
                    </SelectItem>
                  ))}
            ...
            <div className="space-y-2">
              <Label htmlFor="frequency-cap">Intervalo mínimo por cliente</Label>
              <Select name="frequency_cap" defaultValue="24">
                <SelectTrigger id="frequency-cap">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          {scheduleType === "scheduled" ? (
            <div className="space-y-2">
              <Label htmlFor="scheduled-at">Data e hora da loja</Label>
              <Input id="scheduled-at" name="scheduled_at" type="datetime-local" required />
            </div>
          ) : null}
          {scheduleType === "recurring" ? (
            <div className="grid gap-3">
              ...
              <Input name="recurrence_time" type="time" defaultValue="10:00" className="max-w-40" />
            </div>
          ) : null}
          {scheduleType === "automatic" ? (
            <p className="rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
              O envio acontecerá automaticamente quando o cliente entrar no público escolhido.
            </p>
          ) : null}
