...
export function CourierPwaLogin(props: CourierPwaLoginProps) {
  return (
    <main className="min-h-dvh bg-background px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(2rem,env(safe-area-inset-top))] text-foreground">
      <div className="mx-auto flex min-h-[calc(100dvh-3.5rem)] w-full max-w-sm flex-col">
        <header className="flex flex-col items-center pt-[7dvh] text-center">
          ...
          />
          <p className="mt-5 text-lg font-black tracking-tight">Pedi Um <span className="text-primary">Entregadores</span></p>
            ...
            <form onSubmit={props.onRecover} className="flex flex-1 flex-col" noValidate>
              <div className="space-y-2">
                ...
                <div className="relative"><Mail className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-slate-400" />
                  <Input id="courier-recovery" type="email" autoComplete="email" value={props.identifier} onChange={(event) => props.onIdentifierChange(event.target.value)} className="h-14 rounded-2xl border-primary/20 bg-card pl-12 text-base" placeholder="voce@email.com" autoFocus required />
                </div>
              </div>
              <div className="mt-auto space-y-3 pt-8">
                <Button className="h-14 w-full rounded-2xl text-base" disabled={props.loading}>{props.loading ? "Enviando…" : "Enviar recuperação"}</Button>
            ...
            <form onSubmit={props.onLogin} className="flex flex-1 flex-col" noValidate>
              <div className="space-y-5">
                <div className="space-y-2"><Label htmlFor="courier-login">E-mail ou telefone</Label>
                  <div className="relative"><Mail className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-slate-400" />
                    <Input id="courier-login" autoComplete="username" value={props.identifier} onChange={(event) => props.onIdentifierChange(event.target.value)} className="h-14 rounded-2xl border-primary/20 bg-card pl-12 text-base" placeholder="Seu acesso" autoFocus required />
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between"><Label htmlFor="courier-password">Senha</Label><Button type="button" variant="link" className="h-auto p-0 text-xs text-primary" onClick={() => props.onSetRecovering(true)}>Esqueci a senha</Button></div>
                  <div className="relative"><KeyRound className="pointer-events-none absolute left-4 top-1/2 size-5 -translate-y-1/2 text-slate-400" />
                    <Input id="courier-password" type={props.showPassword ? "text" : "password"} autoComplete="current-password" value={props.password} onChange={(event) => props.onPasswordChange(event.target.value)} className="h-14 rounded-2xl border-primary/20 bg-card px-12 text-base" required />
                ...
                {props.otpSent ? <div className="space-y-3 rounded-2xl border border-primary/20 bg-card p-4"><Label htmlFor="courier-otp">Código enviado por e-mail</Label><Input id="courier-otp" inputMode="numeric" autoComplete="one-time-code" maxLength={8} value={props.otpCode} onChange={(event) => props.onOtpCodeChange(event.target.value)} className="h-12 text-center text-lg" /><Button type="button" variant="outline" className="h-12 w-full" onClick={props.onVerifyOtp}>Confirmar código</Button></div> : null}
              </div>
              <div className="mt-auto space-y-3 pt-8">
                <Button className="h-14 w-full rounded-2xl text-base font-bold" disabled={props.loading}>{props.loading ? "Entrando…" : "Entrar como entregador"}</Button>
                {!props.otpSent ? <Button type="button" variant="outline" className="h-14 w-full rounded-2xl border-primary/30 bg-card" onClick={props.onSendOtp}><Mail className="size-5 text-primary" /> Entrar com código por e-mail</Button> : null}
